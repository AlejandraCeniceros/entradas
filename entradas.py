...
def actualizar(window):
    ...
    cantidadmovimiento = 0.2 * tiempoDiferencial
    
    if estado_tecla_izquierda == glfw.PRESS:
        pos_x_tr = pos_x_tr - cantidadmovimiento
    if estado_tecla_derecha == glfw.PRESS:
        pos_x_tr = pos_x_tr + cantidadmovimiento
    if estado_tecla_abajo == glfw.PRESS:
        pos_y_tr = pos_y_tr - cantidadmovimiento
...
def dibujar():
    ...
    global pos_y_tr
    glPushMatrix()
    glTranslate(pos_x_tr, pos_y_tr, 0)
    glColor3f(0,1,0)
...
def key_callback(window, key, scancode, action, mods):
    global pos_x_tr
    global pos_y_tr
    #eventos: press, release, repeat
    #if key == glfw.KEY_ESCAPE:
    #   if action == glfw.PRESS:
    #       print("Se dectecto un press en tecla ESC")
    #    if action == glfw.RELEASE:
    #        print("Se dectecto un release en tecla ESC")
    #    if action == glfw.REPEAT:
    #       print("Se dectecto un repeat en tecla ESC")
    ...
    if action == glfw.PRESS or action == glfw.REPEAT:
        ...
        if key == glfw.KEY_LEFT:
            pos_x_tr = pos_x_tr - 0.05
            
        if key == glfw.KEY_RIGHT:
            pos_x_tr = pos_x_tr + 0.05

        if key == glfw.KEY_DOWN:
            pos_y_tr = pos_y_tr - 0.05
